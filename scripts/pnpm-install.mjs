...
export class InstallProgress {
  ...
  accept(event) {
    ...
    if (event?.name === 'pnpm:stage' && event.stage === 'importing_done' && event.prefix === this.project)
      this.complete = true;
    if (
...
function showLine(line, progress, failure) {
  ...
  if (event?.level === 'error' && failure.code !== 65) {
    failure.code = event.name === 'pnpm' && OPERATIONAL_FAILURE_CODES.has(event.err?.code) ? 70 : 65;
  }
