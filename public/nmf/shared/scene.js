...
export function setupTheme() {
  ...
  try {
    ...
    sync();
    new MutationObserver(sync).observe(pbody, { attributes: true, attributeFilter: ['data-md-color-scheme'] });
  } catch (_) {
    /* cross-origin / standalone: keep the dark default */
  }
}

export function loadModel(mj, path) {
  if (typeof mj.mj_loadXML === 'function') return mj.mj_loadXML(path);
  if (mj.MjModel && typeof mj.MjModel.from_xml_path === 'function') return mj.MjModel.from_xml_path(path);
...
export async function writeModelToFS(mj, { xmlText, xmlName = 'model.xml', modelBaseUrl, onProgress }) {
  const meshFiles = [...new Set([...xmlText.matchAll(/<mesh[^>]*\bfile="([^"]+)"/g)].map((m) => m[1]))];
  try {
    mj.FS.mkdir('/work');
  } catch (_) {
    /* already exists */
...
export function pool(make) {
  ...
  return {
    ...
    next() {
      const o = items[cur] || (items[cur] = make());
      cur++;
      return o;
    },
    end() {
      for (let i = cur; i < items.length; i++) items[i].visible = false;
    },
  };
}
